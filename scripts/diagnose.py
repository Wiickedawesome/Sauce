#!/usr/bin/env python3
"""
scripts/diagnose.py — Query database for current system health state.

Usage: python scripts/diagnose.py
"""

import json
import sys
from datetime import UTC, datetime
from pathlib import Path

sys.path.insert(0, str(Path(__file__).parent.parent))

from sqlalchemy import text
from sqlalchemy.exc import SQLAlchemyError

from sauce.adapters.db import get_session
from sauce.core.config import get_settings


def _parse_payload(raw: object) -> dict[str, object]:
    if isinstance(raw, dict):
        return raw
    if not raw:
        return {}
    try:
        parsed = json.loads(str(raw))
    except (TypeError, ValueError, json.JSONDecodeError):
        return {}
    return parsed if isinstance(parsed, dict) else {}


def _safe_fetchone(session, query: str, params: dict[str, object] | None = None):
    try:
        return session.execute(text(query), params or {}).fetchone()
    except SQLAlchemyError as exc:
        print(f"  Query unavailable: {exc}")
        return None


def _safe_fetchall(session, query: str, params: dict[str, object] | None = None):
    try:
        return session.execute(text(query), params or {}).fetchall()
    except SQLAlchemyError as exc:
        print(f"  Query unavailable: {exc}")
        return []


def run() -> None:
    settings = get_settings()
    session = get_session()

    try:
        dialect = session.bind.dialect.name if session.bind is not None else "unknown"
        if dialect == "postgresql":
            db_type = "Supabase (PostgreSQL)"
        elif dialect == "sqlite":
            db_type = f"SQLite ({settings.db_path})"
        else:
            db_type = dialect

        print("=" * 70)
        print("SAUCE SYSTEM DIAGNOSTIC")
        print(f"DB: {db_type}")
        print("=" * 70)

        # ── 1. Pause state ────────────────────────────────────────────────────
        print("\n[PAUSE STATE]")
        current_pause = settings.trading_pause
        status = "*** PAUSED ***" if current_pause else "NOT PAUSED"
        print(f"  status   : {status}")
        print("  source   : env/config")
        print(f"  TRADING_PAUSE env/config: {current_pause}")

        rows = _safe_fetchall(
            session,
            "SELECT payload, timestamp FROM audit_events WHERE event_type = 'safety_check' ORDER BY timestamp DESC LIMIT 20",
        )
        halt_row = None
        for payload_raw, ts in rows:
            payload = _parse_payload(payload_raw)
            if payload.get("action") == "halt":
                halt_row = (payload, ts)
                break
        if halt_row:
            payload, ts = halt_row
            print(f"  last_halt_action: {payload.get('action')}")
            print(f"  last_halt_reason: {payload.get('reason')!r}")
            print(f"  last_halt_at    : {ts}")
        else:
            print("  last_halt_action: none recorded")

        # ── 2. Recent ops summaries ───────────────────────────────────────────
        print("\n[TODAY SUMMARY]")
        today = datetime.now(UTC).strftime("%Y-%m-%d")
        row = _safe_fetchone(
            session,
            (
                "SELECT loop_runs, signals_fired, signals_skipped, orders_placed, trades_closed, "
                "realized_pnl_usd, ending_equity, regime, updated_at "
                "FROM daily_summary WHERE date = :today LIMIT 1"
            ),
            {"today": today},
        )
        if not row:
            print("  No daily summary row found for today")
        else:
            (
                loop_runs,
                signals_fired,
                signals_skipped,
                orders_placed,
                trades_closed,
                realized_pnl_usd,
                ending_equity,
                regime,
                updated_at,
            ) = row
            print(f"  loop_runs      : {loop_runs}")
            print(f"  signals_fired  : {signals_fired}")
            print(f"  signals_skipped: {signals_skipped}")
            print(f"  orders_placed  : {orders_placed}")
            print(f"  trades_closed  : {trades_closed}")
            print(f"  realized_pnl   : ${float(realized_pnl_usd or 0):,.2f}")
            print(f"  ending_equity  : ${float(ending_equity or 0):,.2f}")
            print(f"  regime         : {regime}")
            print(f"  updated_at     : {updated_at}")

        # ── 3. Recent supervisor decisions ───────────────────────────────────
        print("\n[LAST 10 SUPERVISOR DECISIONS]")
        rows = _safe_fetchall(
            session,
            "SELECT timestamp, payload FROM audit_events WHERE event_type = 'supervisor_decision' ORDER BY timestamp DESC LIMIT 10",
        )
        if not rows:
            print("  (none found)")
        for ts, payload_raw in rows:
            payload = _parse_payload(payload_raw)
            action = payload.get("action")
            reason = payload.get("reason")
            print(f"  {ts}  action={str(action):7s}  reason={reason!r}")

        # ── 4. Recent loop errors ─────────────────────────────────────────────
        print("\n[LAST 10 LOOP-LEVEL ERRORS]")
        rows = _safe_fetchall(
            session,
            "SELECT timestamp, symbol, payload FROM audit_events WHERE event_type = 'error' ORDER BY timestamp DESC LIMIT 10",
        )
        if not rows:
            print("  (none)")
        for ts, sym, payload_raw in rows:
            payload = _parse_payload(payload_raw)
            stage = payload.get("stage")
            err = payload.get("error")
            print(f"  {ts}  symbol={str(sym):12s}  stage={str(stage):25s}  error={str(err)[:80]!r}")

        # ── 5. Recent signals ─────────────────────────────────────────────────
        print("\n[LAST 15 RESEARCH SIGNALS]")
        rows = _safe_fetchall(
            session,
            "SELECT timestamp, symbol, side, score, threshold, strategy_name FROM signal_log ORDER BY timestamp DESC LIMIT 15",
        )
        if not rows:
            print("  (none found)")
        for ts, sym, side, score, threshold, strategy_name in rows:
            print(
                f"  {ts}  symbol={sym!s:12s}  side={side!s:6s}  "
                f"score={int(score):3d}/{int(threshold):3d}  strategy={strategy_name!s}"
            )

        # ── 6. Latest loop_end timestamps ─────────────────────────────────────
        print("\n[LAST 5 LOOP RUNS]")
        rows = _safe_fetchall(
            session,
            (
                "SELECT s.loop_id, s.timestamp, e.timestamp, e.payload "
                "FROM audit_events s "
                "LEFT JOIN audit_events e ON e.loop_id = s.loop_id AND e.event_type = 'loop_end' "
                "WHERE s.event_type = 'loop_start' "
                "ORDER BY s.timestamp DESC LIMIT 5"
            ),
        )
        if not rows:
            print("  (none found)")
        for loop_id, start_ts, end_ts, payload_raw in rows:
            payload = _parse_payload(payload_raw)
            status_value = payload.get("status") if payload else None
            print(
                f"  start={start_ts}  end={end_ts}  status={status_value or 'missing'}  loop_id={loop_id}"
            )
    finally:
        session.close()

    print("\n" + "=" * 70)
    print("To resume trading:  set TRADING_PAUSE=false in .env and restart")
    print("=" * 70)


if __name__ == "__main__":
    run()
